...
public interface IOrganicManureLogic
{
    ...
    Task<(List<int>, Error)> FetchManureTypsIdsByManIdFromOrgManure(int managementId);
    ...
    Task<(decimal, Error)> FetchTotalNBasedOnCropIdFromOrgManureAndFertiliser(int cropId, bool confirm, int? fertiliserId, int? organicManureId);
    ...
    Task<(string?, Error?)> FetchOrganicManureClosedPeriod(int soilTypeId, int fieldType, int harvestYear, DateTime? sowingDate, int countryId, int? cropGroupId, int? cropTypeId, bool isPerennial);
}
