     ...
     IFieldLogicDependencies logicDependencies) : Controller
{
    ...
    private const string _addFieldErrorTempData = "AddFieldError";
    private const string _errorTempData = "Error";
    private const string _manureNonSpreadingAreaModelStateKey = "ManureNonSpreadingArea";
    private const string _croppedAreaModelStateKey = "CroppedArea";
    private const string _totalAreaModelStateKey = "TotalArea";
    private const string _elevationFieldActionName = "ElevationField";
    ...
    public async Task<IActionResult> CreateFieldCancel(string id, string? q)
    {
        _logger.LogTrace("Field Controller : CreateFieldCancel({Id}) action called", id);
    ...
    {
        _logger.LogTrace("Field Controller : BackActionForAddField({Id}) action called", id);
    ...
    /// <summary>
    /// Add new field on farm
    /// </summary>
    /// <param name="q">Encrypted FarmId</param>
    /// <param name="r">Encrypted Year</param>
    /// <returns></returns>
    [HttpGet]
    public async Task<IActionResult> AddField(string q, string? r)
    {
        ...
        {
            _logger.LogTrace(ex, "Field Controller : Exception in BackActionForAddField() action : {Message}, {StackTrace}", ex.Message, ex.StackTrace);
            TempData[_errorTempData] = string.Concat(error?.Message == null ? "" : error.Message, ex.Message);
    ...
    public async Task<IActionResult> AddField(FieldViewModel model)
    {
        _logger.LogTrace("Field Controller : AddField() action called");
        await ValidateField(model);
        ...
        if (model.CopyExistingField != null && (model.CopyExistingField.Value))
        {
            ...
            if (fieldResponse != null && error == null)
            {
                ...
                model.TotalArea = fieldResponse.Field.TotalArea;
                model.CroppedArea = fieldResponse.Field.CroppedArea;
                model.LPIDNumber = fieldResponse.Field.LPIDNumber;
                model.ManureNonSpreadingArea = fieldResponse.Field.ManureNonSpreadingArea;
                model.NVZProgrammeID = fieldResponse.Field.NVZProgrammeID;
                model.IsWithinNVZ = fieldResponse.Field.IsWithinNVZ;
                model.IsAbove300SeaLevel = fieldResponse.Field.IsAbove300SeaLevel;
                ...
                model.SoilType = soilType?.SoilType;
                SetFieldDataToSession(model);
            }
            else
            {
                TempData[_addFieldErrorTempData] = error.Message;
    ...
    private async Task ValidateField(FieldViewModel model)
    {
    ...
    public async Task<IActionResult> FieldMeasurements(FieldViewModel field)
    {
        ...
        ValidateFieldMeasurements(field);
    ...
    private void ValidateFieldMeasurements(FieldViewModel field)
    {
        ValidateTotalArea(field);
        ValidateCroppedArea(field);
        ValidateManureNonSpreadingArea(field);
    }

    private void ValidateManureNonSpreadingArea(FieldViewModel field)
    {
        if ((!ModelState.IsValid) && ModelState.ContainsKey(_manureNonSpreadingAreaModelStateKey))
        {
            var InvalidFormatError = ModelState[_manureNonSpreadingAreaModelStateKey]?.Errors.Count > 0 ?
                            ModelState[_manureNonSpreadingAreaModelStateKey]?.Errors[0].ErrorMessage.ToString() : null;

            if (InvalidFormatError != null && InvalidFormatError.Equals(string.Format(Resource.lblEnterNumericValue, ModelState[_manureNonSpreadingAreaModelStateKey].AttemptedValue, Resource.lblManureNonSpreadingArea)))
            {
                ModelState[_manureNonSpreadingAreaModelStateKey]?.Errors.Clear();
                ModelState[_manureNonSpreadingAreaModelStateKey]?.Errors.Add(string.Format(Resource.MsgEnterDataOnlyInNumber, Resource.lblArea));
            }
        }
    ...
    {
        if ((!ModelState.IsValid) && ModelState.ContainsKey(_croppedAreaModelStateKey))
        {
            var InvalidFormatError = ModelState[_croppedAreaModelStateKey]?.Errors.Count > 0 ?
                            ModelState[_croppedAreaModelStateKey]?.Errors[0].ErrorMessage.ToString() : null;

            if (InvalidFormatError != null && InvalidFormatError.Equals(string.Format(Resource.lblEnterNumericValue, ModelState[_croppedAreaModelStateKey].AttemptedValue, Resource.lblCroppedArea)))
            {
                ModelState[_croppedAreaModelStateKey]?.Errors.Clear();
                ModelState[_croppedAreaModelStateKey]?.Errors.Add(string.Format(Resource.MsgEnterDataOnlyInNumber, Resource.lblArea));
            }
        }
    ...
    {
        if ((!ModelState.IsValid) && ModelState.ContainsKey(_totalAreaModelStateKey))
        {
            var InvalidFormatError = ModelState[_totalAreaModelStateKey]?.Errors.Count > 0 ?
                            ModelState[_totalAreaModelStateKey]?.Errors[0].ErrorMessage.ToString() : null;

            if (InvalidFormatError != null && InvalidFormatError.Equals(string.Format(Resource.lblEnterNumericValue, ModelState[_totalAreaModelStateKey].AttemptedValue, Resource.lblTotalFieldArea)))
            {
                ModelState[_totalAreaModelStateKey]?.Errors.Clear();
                ModelState[_totalAreaModelStateKey]?.Errors.Add(string.Format(Resource.MsgEnterDataOnlyInNumber, Resource.lblArea));
            }
        }
    ...
    public async Task<IActionResult> SoilType()
    {
        ...
        try
        {
            ...
            if (ViewBag.SoilTypesList == null && ((List<SoilTypesResponse>)ViewBag.SoilTypesList).Count > 0)
            {
                ...
                RedirectToAction(_elevationFieldActionName);
            }
        ...
        {
            _logger.LogTrace(ex, "Field Controller : Exception in SoilType() action : {Message}, {StackTrace}", ex.Message, ex.StackTrace);
    ...
    public async Task<IActionResult> SoilType(FieldViewModel model)
    {
        ...
        try
        {
            ...
            if (fieldData.SoilTypeID != (int)NMP.Commons.Enums.SoilTypeEngland.DeepClayey &&
                model.SoilTypeID == (int)NMP.Commons.Enums.SoilTypeEngland.DeepClayey)
            {
            ...
            if (model.IsCheckAnswer && (!isSoilTypeChange))
            {
                model.IsSoilReleasingClay = false;
    ...
    public async Task<IActionResult> SulphurDeficient(FieldViewModel model)
    {
        ...
        return await Task.FromResult(RedirectToAction("SoilDate"));
    }
        ...
        var invalidDateMessages = new HashSet<string>
{
    ...
    public IActionResult SoilNutrientValueType(FieldViewModel model)
    {
        ...
        if (model.SoilAnalyses.SoilNutrientValueType == null)
        {
            ModelState.AddModelError("SoilAnalyses.SoilNutrientValueType", Resource.MsgSelectAnOptionBeforeContinuing);
        }
        ...
        if (model.IsCheckAnswer)
        {
            FieldViewModel? fieldViewModel = LoadFieldDataFromSession();
            if (fieldViewModel != null && fieldViewModel.SoilAnalyses != null && fieldViewModel.SoilAnalyses.SoilNutrientValueType == model.SoilAnalyses.SoilNutrientValueType)
            {
                model.SoilAnalyses.IsSoilNutrientValueTypeChange = false;
                SetFieldDataToSession(model);
                return RedirectToAction(_checkAnswerActionName);
            }
            else
            {
                model.SoilAnalyses.IsSoilNutrientValueTypeChange = true;
                SetFieldDataToSession(model);
            }
        }
    ...
    public async Task<IActionResult> SoilNutrientValue(FieldViewModel model)
    {
        ...
        try
        {
            ...
            return model.IsCheckAnswer
                ...
                : RedirectToAction(_lastHarvestYearActionName);
        }
        catch (Exception ex)
        {
            _logger.LogTrace(
                ex,
                "Field Controller : Exception in SoilNutrientValue() post action : {Message}, {StackTrace}",
                ex.Message,
                ex.StackTrace);
    ...
    private static void ClearNutrientValues(FieldViewModel model)
    {

        if (model.SoilAnalyses.SoilNutrientValueType == (int)NMP.Commons.Enums.SoilNutrientValueType.Index)
        {
            model.SoilAnalyses.Phosphorus = null;
            model.SoilAnalyses.Magnesium = null;
            model.SoilAnalyses.Potassium = null;
            model.SoilAnalyses.MagnesiumStatus = null;
            model.SoilAnalyses.PhosphorusStatus = null;
            model.SoilAnalyses.PotassiumStatus = null;
        }
        else if (model.SoilAnalyses.SoilNutrientValueType == (int)NMP.Commons.Enums.SoilNutrientValueType.Status)
        {
            model.SoilAnalyses.Phosphorus = null;
            model.SoilAnalyses.Magnesium = null;
            model.SoilAnalyses.Potassium = null;
        ...
        else if (model.SoilAnalyses.SoilNutrientValueType == (int)NMP.Commons.Enums.SoilNutrientValueType.Miligram)
        {
            model.SoilAnalyses.PhosphorusStatus = null;
            model.SoilAnalyses.PotassiumStatus = null;
            model.SoilAnalyses.MagnesiumStatus = null;
    ...
    private void ValidateSoilNutrientValues(FieldViewModel model)
    {
        if (model.SoilAnalyses.SoilNutrientValueType.HasValue)
        {
            ...
            else if (model.SoilAnalyses.SoilNutrientValueType.Value == (int)NMP.Commons.Enums.SoilNutrientValueType.Miligram)
            {
                ValidateSoilNutrientValuesData(model);
            }
            else if (model.FarmRB209CountryID.HasValue && model.FarmRB209CountryID.Value == (int)NMP.Commons.Enums.RB209Country.Scotland && (model.SoilAnalyses.SoilNutrientValueType.Value == (int)NMP.Commons.Enums.SoilNutrientValueType.Status || model.SoilAnalyses.SoilNutrientValueType.Value == (int)NMP.Commons.Enums.SoilNutrientValueType.Index))
            {
    ...
    public async Task<IActionResult> CropGroups()
    {
        ...
        {
            _logger.LogTrace(ex, "Field Controller : Exception in CropGroups() action : {Message}, {StackTrace}", ex.Message, ex.StackTrace);
    ...
    public async Task<IActionResult> BackCheckAnswer()
    {
        ...
        if (model.PreviousCroppings.HasGrassInLastThreeYear != null && model.PreviousCroppings.HasGrassInLastThreeYear.Value)
        {
            ...
            else
            {
                ...
                {
                    return await Task.FromResult(RedirectToAction("SoilNitrogenSupplyItems"));
                }
                else
                {
                    return await Task.FromResult(RedirectToAction("HasGreaterThan30PercentClover"));
                }
            }
        }
    ...
    public async Task<IActionResult> CheckAnswer()
    {
        ...
        try
        {
            ...
            model.SoilAnalyses.IsSoilAnalysesMethodChange = false;
            model.SoilAnalyses.IsSoilNutrientValueTypeChange = false;
    ...
    public async Task<IActionResult> CheckAnswer(FieldViewModel model)
    {
        ...
        try
        {
            ...
            soilAnalysis = BindSoilAnalysisData(model, userId, soilAnalysis);
            FieldData fieldData = FieldDataForSave(model, userId, previousCropping, soilAnalysis);
    ...
    static FieldData FieldDataForSave(FieldViewModel model, int userId, List<PreviousCroppingData> previousCropping, SoilAnalysis? soilAnalysis)
    {
    ...
    static SoilAnalysis? BindSoilAnalysisData(FieldViewModel model, int userId, SoilAnalysis? soilAnalysis)
    {
        if (model.RecentSoilAnalysisQuestion.HasValue && model.RecentSoilAnalysisQuestion.Value)
        {
            soilAnalysis = new SoilAnalysis()
            {
                ...
                PotassiumMethodologyID = model.SoilAnalyses.PotassiumMethodologyID,
                PotassiumStatus = model.SoilAnalyses.PotassiumStatus,
                MagnesiumMethodologyID = model.SoilAnalyses.MagnesiumMethodologyID,
    ...
    static void BindPotassium(FieldViewModel model)
    {
        if (!string.IsNullOrWhiteSpace(model.PotassiumIndexValue))
        {
    ...
    static void BindPrevCroppingDataForSave(FieldViewModel model, List<PreviousCroppingData> previousCropping)
    {
        var newPreviousCropping = new PreviousCroppingData
        {
            CropGroupID = model.CropGroupId,
    ...
    static void BindPreviousCroppingData(FieldViewModel model, List<PreviousCroppingData> previousCropping)
    {
    ...
    async Task<IActionResult> GetDataForInvalidModelState(FieldViewModel model)
    {
        await FetchSelectedNVZName(model);
        await FetchMethodologyName(model);
        await FetchPscIndexName(model);
    ...
    private async Task FetchMethodologyName(FieldViewModel model)
    {
        int phosphorusId = 1;
        ...
        if (nutrients != null && nutrients.Count > 0)
        {
            ...
            (SoilMethologiesResponse? soilMethology, error) = await _soilService.FetchSoilMethodologyNameByNutrientIdAndMethodologyId(phosphorusId, model.SoilAnalyses.PhosphorusMethodologyID ?? 0);
    ...
    private void ValidateSoilAnalysis(FieldViewModel model)
    {
        ...
        if (model.SoilAnalyses.SoilNutrientValueType.HasValue)
        {
                    ...
                    !model.SoilAnalyses.Phosphorus.HasValue && !model.SoilAnalyses.Magnesium.HasValue))
            {
                ...
                !model.SoilAnalyses.PH.HasValue && string.IsNullOrWhiteSpace(model.PotassiumIndexValue) &&
                    !model.SoilAnalyses.MagnesiumIndex.HasValue && !model.SoilAnalyses.PhosphorusIndex.HasValue)
            {
               ...
               !model.SoilAnalyses.PH.HasValue && string.IsNullOrWhiteSpace(model.SoilAnalyses.PotassiumStatus) &&
                   string.IsNullOrWhiteSpace(model.SoilAnalyses.PhosphorusStatus) && string.IsNullOrWhiteSpace(model.SoilAnalyses.MagnesiumStatus))
            {
                ValidateSoilAnalysisIndexValues(model);
            }
        }
    ...
    private void ValidateGrassJourneyFields(FieldViewModel model)
    {
        ...
        if (model.FarmRB209CountryID != (int)NMP.Commons.Enums.RB209Country.Scotland)
        {
            ValidateGrassJourneyFieldsForEngland(model, hasGrass);
        }
        else
        {
    ...
    private void ValidateGrassJourneyFieldsForEngland(FieldViewModel model, bool? hasGrass)
    {
    ...
    public async Task<IActionResult> ManageFarmFields(string id, string? q, string? name, string? isDeleted)
    {
        ...
        {
            int farmId = Convert.ToInt32(_farmDataProtector.Unprotect(id));
            model.Fields = await _fieldLogic.FetchFieldsByFarmId(farmId);
    ...
    public async Task<IActionResult> FieldSoilAnalysisDetail(string farmId, string fieldId, string? q, string? r, string? s, string? t)//id encryptedFieldId,farmID=EncryptedFarmID,q=success,r=FiedlOrSoilAnalysis,s=soilUpdateOrSave
    {
        ...
        Error? error = new Error();
        (error, int decryptedFieldId) = await FieldSoilAnalysisModelBinding(farmId, fieldId, t, model, error);
        List<SoilAnalysisResponse> soilAnalysisResponse = await SoilAnalysisListViewBag(decryptedFieldId);
        await BindSuccessViewBag(q, r, s, model, soilAnalysisResponse);
    ...
    private async Task BindSuccessViewBag(string? q, string? r, string? s, FieldViewModel model, List<SoilAnalysisResponse> soilAnalysisResponse)
    {
        if (!string.IsNullOrWhiteSpace(q))
        {
            ...
            if (!string.IsNullOrWhiteSpace(r))
            {
                ...
                {
                    await SetSuccessContent(q, s, model, soilAnalysisResponse, statusFor);
    ...
    private async Task SetSuccessContent(string q, string? s, FieldViewModel model, List<SoilAnalysisResponse> soilAnalysisResponse, string statusFor)
    {
        ...
        else if (statusFor == Resource.lblSoilAnalysis)
        {
            ...
            else
            {
                ...
                GetSuccessMessage(s, soilAnalysisResponse, crop);
            }
        }
    }

    void GetSuccessMessage(string? s, List<SoilAnalysisResponse> soilAnalysisResponse, List<Crop> crop)
    {
        if (crop != null && crop.Count > 0)
        {
            ...
            {
                SuccessMessageWithSoilAnalysis(s, soilAnalysisResponse, crop);
            }
    ...
    async Task<(Error error, int decryptedFieldId)> FieldSoilAnalysisModelBinding(string farmId, string fieldId, string? t, FieldViewModel model, Error error)
    {
        ...
        if (isPreviousCroppingBindRequired)
        {
            ...
            {
                model.LastHarvestYear = cropPlans.Any() ? cropPlans.Min(cp => cp.Year) - 1 : (model.LastHarvestYear ?? 0);
                error = await BindingPreviousCroppingDataForScotland(model, error, decryptedFieldId);
            }
        }
        ...
        List<SoilTypesResponse> soilTypes = await _fieldLogic.FetchSoilTypes();
        SoilReleasingClayViewBag(model, field, soilTypes);
        model.EncryptedFarmId = farmId;
        model.FarmName = farm?.Name;
        return (error, decryptedFieldId);
    }

    private async Task<Error> BindingPreviousCroppingDataForScotland(FieldViewModel model, Error error, int decryptedFieldId)
    {
        (PreviousCropping? prevCroppingData, error) = await _previousCroppingLogic.FetchDataByFieldIdAndYear(decryptedFieldId, model.LastHarvestYear.Value);
    ...
    async Task<List<SoilAnalysisResponse>> SoilAnalysisListViewBag(int decryptedFieldId)
    {
    ...
    private async Task PreviousCroppingViewBagBind(FieldViewModel model, Error error, List<Crop> cropPlans, List<PreviousCroppingData> prevCroppings)
    {
        if (string.IsNullOrWhiteSpace(error?.Message))
        {
            ...
            await PrevCroppingExcludingPlans(cropPlans, prevCroppings);
            ...
            {
                await GrassManagementSoilNitrogenSupplyViewBag(prevCroppings);
            }
    ...
    private void SoilReleasingClayViewBag(FieldViewModel model, Field field, List<SoilTypesResponse> soilTypes)
    {
    ...
    async Task GrassManagementSoilNitrogenSupplyViewBag(List<PreviousCroppingData> prevCroppings)
    {
        List<CommonResponse> grassManagements = await _fieldLogic.GetGrassManagementOptions();
    ...
    private async Task<(Error error, List<PreviousCroppingData> prevCroppings, bool isPreviousCroppingBindRequired)> IsPreviousCroppingBindRequiredSetFlag(FieldViewModel model, Error error, int decryptedFieldId, List<Crop> cropPlans, List<PreviousCroppingData> prevCroppings, bool isPreviousCroppingBindRequired)
    {
        if (!cropPlans.Any())
        {
    ...
    public IActionResult RecentSoilAnalysisQuestion()
    {
        ...
        {
            _logger.LogTrace(ex, "Field Controller : Exception in RecentSoilAnalysisQuestion() action : {Message}, {StackTrace}", ex.Message, ex.StackTrace);
            TempData[_errorTempData] = ex.Message;
            return RedirectToAction("SoilType");
        }
        return View(model);
    ...
    public IActionResult RecentSoilAnalysisQuestion(FieldViewModel model)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                model.SoilAnalyses.PhosphorusIndex = null;
                model.SoilAnalyses.SoilNutrientValueType = null;
                SetFieldDataToSession(model);
        ...
        {
            _logger.LogTrace(ex, "Field Controller : Exception in RecentSoilAnalysisQuestion() post action : {Message}, {StackTrace}", ex.Message, ex.StackTrace);
            TempData[_errorTempData] = ex.Message;
            return View(model);
        }
    }
    ...
    private async Task<(Error error, PreviousCropping? prevCroppingDataForScotland)> BindingPreviousCropDataForUpdateFieldScotland(FieldViewModel model, Error error, int decrptedFieldId, PreviousCropping? prevCroppingDataForScotland)
    {
        (prevCroppingDataForScotland, error) = await _previousCroppingLogic.FetchDataByFieldIdAndYear(decrptedFieldId, model.LastHarvestYear.Value);
        if (prevCroppingDataForScotland != null)
        {
    ...
    public async Task<IActionResult> UpdateField(string? fieldId, string? farmId)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                cropPlans = await GetCropPlans(model, cropPlans);
    ...
    public async Task<IActionResult> UpdateField(FieldViewModel model)
    {
        ...
        try
        {
            ...
            await BindPreviousCroppingsListForUpdateFieldScotland(model, fieldId);
            ...
            if (!onlyFieldUpdate)
            {
                ...
                (fieldResponse, error) =
                await _fieldLogic.UpdateFieldAsync(fieldData, fieldId);
            }
            else
            {
                field.ID = fieldId;
                (fieldResponse, error) =
                await _fieldLogic.UpdateFieldDataAsync(field);
            }


            if (string.IsNullOrWhiteSpace(error?.Message) && fieldResponse != null)
            {
    ...
    private async Task BindPreviousCroppingsListForUpdateFieldScotland(FieldViewModel model, int fieldId)
    {
        if (model.FarmRB209CountryID == (int)NMP.Commons.Enums.RB209Country.Scotland)
        {
            (PreviousCropping? preCropping, _) = await _previousCroppingLogic.FetchDataByFieldIdAndYear(fieldId, model.LastHarvestYear.Value);
            var newPreviousCropping = new PreviousCroppingData
            {
                ID = preCropping != null ? preCropping.ID : null,
                FieldID = fieldId,
                CropGroupID = model.CropGroupId,
                CropTypeID = model.CropTypeID,
                HasGrassInLastThreeYear = model.CropTypeID == (int)NMP.Commons.Enums.CropTypes.Grass,
    ...
    async Task<(List<Crop> cropPlans, Error error, int decrptedFieldId, bool? hasGrassInLastThreeYear, List<PreviousCroppingData> prevCroppings, bool isPreviousCroppingBindRequired)> PreviousCroppingsByFieldId(string? fieldId, string farmId, FieldViewModel model)
    {
        ...
        model.FarmRB209CountryID = farm?.RB209CountryID;
        //get plans of field
        List<Crop> cropPlans = await _fieldLogic.FetchCropsByFieldId(decrptedFieldId);
        bool? hasGrassInLastThreeYear = null;
    ...
    async Task ModelInitialisationByFieldId(string? fieldId, string farmId, FieldViewModel model, List<Crop> cropPlans, bool? hasGrassInLastThreeYear, List<PreviousCroppingData> prevCroppings, PreviousCropping? previousCroppingForScotland)
    {
        int decrptedFieldId = Convert.ToInt32(_fieldDataProtector.Unprotect(fieldId));
        ...
        {
            model.CropGroupId = previousCroppingForScotland?.CropGroupID;
            model.CropTypeID = previousCroppingForScotland?.CropTypeID;
        }
        ...
        var soilType = await _fieldLogic.FetchSoilTypeById(field.SoilTypeID.Value);
    ...
    async Task<(Error error, bool? hasGrassInLastThreeYear, List<PreviousCroppingData> prevCroppings)> YearsForGrassAndArablePrevCrop(FieldViewModel model, List<Crop> cropPlans, Error error, int decrptedFieldId, List<PreviousCroppingData> prevCroppings)
    {
        bool? hasGrassInLastThreeYear = null;
    ...
    private void BindPreviousGrassesData(FieldViewModel model)
    {
        if (model.PreviousCroppingsList != null && model.PreviousGrassYears != null)
        {
            ...
            foreach (var pc in model.PreviousCroppingsList)
            {
                if (!model.PreviousGrassYears.Contains(pc.HarvestYear ?? 0) && pc.HarvestYear != model.LastHarvestYear)
                {
                    ...
                    if (existing != null)
                    {
                        pc.Action = (int)NMP.Commons.Enums.Action.Update;
    ...
    public IActionResult CopyExistingField(FieldViewModel field)
    {
        ...
        {
            return RedirectToAction(_addFieldActionName, new { q = field.EncryptedFarmId });
        }
    ...
    public IActionResult HasGrassInLastThreeYear()
    {
        ...
        if (model == null)
        {
            _logger.LogTrace("Field Controller : HasGrassInLastThreeYear() : field data not found in session");
            return Functions.RedirectToErrorHandler((int)HttpStatusCode.Conflict);
        }
    ...
    public async Task<IActionResult> GrassManagementOptions(FieldViewModel model)
    {
        ...
        if (!ModelState.IsValid)
        {
            List<CommonResponse> commonResponses = await _fieldLogic.GetGrassManagementOptions();
    ...
    public Task<IActionResult> HasGreaterThan30PercentClover()
    {
        ...
        if (model == null)
        {
            _logger.LogTrace("Field Controller : HasGreaterThan30PercentClover(): field data not found in session");
            return Task.FromResult<IActionResult>(Functions.RedirectToErrorHandler((int)HttpStatusCode.Conflict));
        }
    ...
    public async Task<IActionResult> SoilNitrogenSupplyItems()
    {
        ...
        if (model == null)
        {
            _logger.LogTrace("Field Controller : SoilNitrogenSupplyItems() : field data not found in session");
            return Functions.RedirectToErrorHandler((int)HttpStatusCode.Conflict);
        }

        List<CommonResponse> commonResponses = await _fieldLogic.GetSoilNitrogenSupplyItems();
    ...
    public async Task<IActionResult> LayDuration()
    {
        ...
        if (model == null)
        {
            _logger.LogTrace("Field Controller : LayDuration() : field data not found in session");
            return Functions.RedirectToErrorHandler((int)HttpStatusCode.Conflict);
        }
    ...
    public IActionResult Cancel()
    {
        ...
        {
            _logger.LogTrace(ex, "Field Controller : Exception in Cancel() action : {Message}, {StackTrace}", ex.Message, ex.StackTrace);
            TempData[_addFieldErrorTempData] = ex.Message;
            return RedirectToAction(_checkAnswerActionName);
        }
    ...
    private async Task SoilAnalysisViewModel(FieldViewModel model)
    {
        var (nutrients, error) = await _fieldLogic.FetchNutrientsAsync();
        if (nutrients != null)
        {
            var nutrientId = nutrients.FirstOrDefault(n => n.nutrient.Equals(Resource.lblPhosphate))?.nutrientId ?? 0;
            (List<SoilMethologiesResponse>? soilMethologiesList, Error? _) = await _soilService.FetchSoilMethodologies(nutrientId, model.FarmRB209CountryID.Value);
            if (error == null && soilMethologiesList != null && soilMethologiesList.Count > 0)
            {
    ...
    public async Task<IActionResult> SoilAnalysesMethod()
    {
        ...
        await SoilAnalysisViewModel(model);
        return View(model);
    }
    ...
    public async Task<IActionResult> SoilAnalysesMethod(FieldViewModel model)
    {
        ...
        if (model.SoilAnalyses.PhosphorusMethodologyID == null)
        {
            ModelState.AddModelError("SoilAnalyses.PhosphorusMethodologyID", Resource.MsgSelectAnOptionBeforeContinuing);
        }
        ...
        {
            FieldViewModel? fieldViewModel = LoadFieldDataFromSession();
            if (fieldViewModel != null && fieldViewModel.SoilAnalyses != null && fieldViewModel.SoilAnalyses.PhosphorusMethodologyID == model.SoilAnalyses.PhosphorusMethodologyID)
            {
                model.SoilAnalyses.IsSoilAnalysesMethodChange = false;
                SetFieldDataToSession(model);
                return await Task.FromResult(RedirectToAction(_checkAnswerActionName));
            ...
            {
                model.SoilAnalyses.IsSoilAnalysesMethodChange = true;
                model.SoilAnalyses.SoilNutrientValueTypeName = null;
                ClearNutrientValues(model);
                model.SoilAnalyses.SoilNutrientValueType = null;
            }
        }
